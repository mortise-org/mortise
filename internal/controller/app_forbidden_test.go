...
func TestEnvResourceErrorMissingNamespaceFastRequeues(t *testing.T) {
	// A Forbidden write into a namespace that does not exist yet races
	// namespace creation itself (the authorizer denies before existence is
	// checked) — the earliest phase of the propagation race.
	...
	if err != nil {
		t.Fatalf("expected nil error for forbidden in missing namespace, got %v", err)
	}
	if res.RequeueAfter != rbacPropagationRequeue {
		t.Fatalf("expected RequeueAfter %v, got %v", rbacPropagationRequeue, res.RequeueAfter)
	}
	...
	if fresh.Status.Phase == mortisev1alpha1.AppPhaseFailed {
		t.Fatal("missing-namespace forbidden must not mark the app Failed")
	}
}

// forbiddenFastRequeue is the shared classifier used by the project and
// preview-environment controllers for their own bootstrap-window writes.
func TestForbiddenFastRequeueClassification(t *testing.T) {
	cases := []struct {
		name        string
		nsAge       time.Duration
		nsName      string
		...
		{"young namespace forbidden", 30 * time.Second, "pj-demo-staging", newForbiddenErr(), true},
		{"old namespace forbidden", rbacPropagationWindow + time.Minute, "pj-demo-staging", newForbiddenErr(), false},
		{"boundary is old", rbacPropagationWindow, "pj-demo-staging", newForbiddenErr(), false},
		{"non-forbidden error", 30 * time.Second, "pj-demo-staging", apierrors.NewServiceUnavailable("down"), false},
		{"missing namespace", 30 * time.Second, "pj-demo-gone", newForbiddenErr(), true},
		{"wrapped forbidden", 30 * time.Second, "pj-demo-staging", fmt.Errorf("copy app env secret: %w", newForbiddenErr()), true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			r, _, _ := forbiddenTestFixtures(t, tc.nsAge)
			res, ok := forbiddenFastRequeue(context.Background(), r.Client, r.clock(), tc.nsName, tc.err)
			if ok != tc.wantRequeue {
				t.Fatalf("fast requeue = %v, want %v", ok, tc.wantRequeue)
			}
			if tc.wantRequeue && res.RequeueAfter != rbacPropagationRequeue {
