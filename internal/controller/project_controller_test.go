...
var _ = Describe("Project Controller", func() {
	...
	Context("when spec.namespaceOverride is set", func() {
		const projectName = "override-new"
		const nsName = "custom-ns"
		...
		It("creates a namespace with the override name and publishes it in status", func() {
			project := &mortisev1alpha1.Project{
				ObjectMeta: metav1.ObjectMeta{Name: projectName},
				Spec:       mortisev1alpha1.ProjectSpec{NamespaceOverride: nsName},
			}
			Expect(k8sClient.Create(ctx, project)).To(Succeed())
			...
			// No namespace under the default name should have been created.
			var defaulted corev1.Namespace
			err = k8sClient.Get(ctx, types.NamespacedName{Name: ProjectNamespace(projectName)}, &defaulted)
			Expect(errors.IsNotFound(err)).To(BeTrue())
	...
	Context("when the override points at an existing foreign namespace and adoption is disabled", func() {
		const projectName = "adopt-target-off"
		const nsName = "taken-ns-off"
		...
		It("fails with NamespaceAlreadyExists when adoption is disabled", func() {
			Expect(k8sClient.Create(ctx, &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					Name:   nsName,
					Labels: map[string]string{"corp": "prior-art"},
				},
			})).To(Succeed())
			...
			// Pre-existing namespace must remain unowned + unlabelled by us.
			var unchanged corev1.Namespace
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: nsName}, &unchanged)).To(Succeed())
			Expect(unchanged.OwnerReferences).To(BeEmpty())
			Expect(unchanged.Labels).NotTo(HaveKey("app.kubernetes.io/managed-by"))
		})
	})

	Context("when the override points at an existing foreign namespace and adoption is enabled", func() {
		const projectName = "adopt-target-on"
		const nsName = "taken-ns-on"
		...
		It("adopts the namespace with owner ref and Adopted condition", func() {
			Expect(k8sClient.Create(ctx, &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					Name:   nsName,
					Labels: map[string]string{"corp": "prior-art"},
			...
			Expect(cond.Reason).To(Equal(ReasonAdopted))

			var adopted corev1.Namespace
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: nsName}, &adopted)).To(Succeed())
			Expect(adopted.OwnerReferences).ToNot(BeEmpty())
			Expect(adopted.OwnerReferences[0].Kind).To(Equal("Project"))
			Expect(adopted.OwnerReferences[0].Name).To(Equal(projectName))
			Expect(adopted.Labels).To(HaveKeyWithValue("app.kubernetes.io/managed-by", "mortise"))
			Expect(adopted.Labels).To(HaveKeyWithValue("mortise.dev/managed-by", "project"))
			// Pre-existing label must not be wiped.
			Expect(adopted.Labels).To(HaveKeyWithValue("corp", "prior-art"))
		})
	})

	Context("when two Projects target the same namespace name", func() {
		const firstProject = "claimer"
		const secondProject = "squatter"
		const nsName = "shared-ns"

		AfterEach(func() {
			for _, n := range []string{firstProject, secondProject} {
				proj := &mortisev1alpha1.Project{}
				if err := k8sClient.Get(ctx, types.NamespacedName{Name: n}, proj); err == nil {
					proj.Finalizers = nil
					_ = k8sClient.Update(ctx, proj)
					_ = k8sClient.Delete(ctx, proj)
				}
		...
		It("fails the second one with NamespaceConflict", func() {
			first := &mortisev1alpha1.Project{
				ObjectMeta: metav1.ObjectMeta{Name: firstProject},
				Spec:       mortisev1alpha1.ProjectSpec{NamespaceOverride: nsName},
			}
			Expect(k8sClient.Create(ctx, first)).To(Succeed())

			r := &ProjectReconciler{Client: k8sClient, Scheme: k8sClient.Scheme()}
			// Drive the first Project to Ready.
			_, err := r.Reconcile(ctx, reconcile.Request{NamespacedName: types.NamespacedName{Name: firstProject}})
			Expect(err).NotTo(HaveOccurred())
			_, err = r.Reconcile(ctx, reconcile.Request{NamespacedName: types.NamespacedName{Name: firstProject}})
			Expect(err).NotTo(HaveOccurred())

			// Now a second Project tries to claim the same namespace name.
			second := &mortisev1alpha1.Project{
				ObjectMeta: metav1.ObjectMeta{Name: secondProject},
				Spec:       mortisev1alpha1.ProjectSpec{NamespaceOverride: nsName},
			}
			Expect(k8sClient.Create(ctx, second)).To(Succeed())

			_, err = r.Reconcile(ctx, reconcile.Request{NamespacedName: types.NamespacedName{Name: secondProject}})
			Expect(err).NotTo(HaveOccurred())
			_, err = r.Reconcile(ctx, reconcile.Request{NamespacedName: types.NamespacedName{Name: secondProject}})
			Expect(err).NotTo(HaveOccurred())

			var updated mortisev1alpha1.Project
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: secondProject}, &updated)).To(Succeed())
			Expect(updated.Status.Phase).To(Equal(mortisev1alpha1.ProjectPhaseFailed))
			cond := findCondition(updated.Status.Conditions, ProjectConditionNamespaceReady)
			Expect(cond).NotTo(BeNil())
			Expect(cond.Reason).To(Equal(ReasonNamespaceConflict))

			// Original owner must still be the first Project.
			var ns corev1.Namespace
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: nsName}, &ns)).To(Succeed())
			Expect(ns.OwnerReferences).To(HaveLen(1))
			Expect(ns.OwnerReferences[0].Name).To(Equal(firstProject))
		})
	})
})
