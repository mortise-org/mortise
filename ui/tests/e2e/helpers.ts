...
export async function getEnvViaAPI(
...
): Promise<Array<{ name: string; value: string; source?: string; bindingRef?: string; bindingKey?: string }>> {
	const res = await request.get(
	...
	return (await res.json()) as Array<{ name: string; value: string; source?: string; bindingRef?: string; bindingKey?: string }>;
}
